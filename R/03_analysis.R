...
labor_trend <- labor_clean %>%
  ...
  pivot_wider(
    ...
    values_from = c(aian_unemp_rate, total_unemp_rate, unemp_gap,
                    aian_lfpr, total_lfpr, lfpr_gap,
                    earnings_ratio),
    names_sep   = "_"   # took some trial and error to get this right
  ) %>%
  mutate(
...
# Look at which states have the worst gaps across dimensions
cat("\n── Rankings by Worst Gap (2023) ──\n")

cat("\nBy unemployment gap (highest = worst):\n")
snapshot_2023 %>% arrange(desc(unemp_gap)) %>%
  select(state_name, unemp_gap) %>% print()

cat("\nBy poverty gap (highest = worst):\n")
snapshot_2023 %>% arrange(desc(poverty_gap)) %>%
  select(state_name, poverty_gap) %>% print()

cat("\nBy ownership gap (highest = worst):\n")
snapshot_2023 %>% arrange(desc(ownership_gap)) %>%
  select(state_name, ownership_gap) %>% print()

cat("\nBy earnings ratio (lowest = worst):\n")
snapshot_2023 %>% arrange(earnings_ratio) %>%
  select(state_name, earnings_ratio) %>% print()

cat("\nSouth Dakota ranks worst across nearly every dimension.\n")
