...
# Colors for AIAN vs total comparison charts
AIAN_COLOR  <- "#1B4F72"
TOTAL_COLOR <- "#AED6F1"
...
fig1 <- ggplot(fig1_data, aes(x = state_name, y = rate, fill = group)) +
  ...
  # Spent a while getting position_dodge spacing to look right
...
fig3 <- ggplot(fig3_data, aes(x = year, y = rate,
  ...
  # Same theme settings as fig1 — copied and adjusted margin
  theme_minimal(base_size = 12) +
  theme(
    plot.title       = element_text(face = "bold", size = 13, hjust = 0),
    plot.subtitle    = element_text(size = 10, color = "gray40", hjust = 0),
...
# Found this centroid approach on Stack Overflow for labeling sf polygons
