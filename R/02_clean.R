...
# Returns number of flagged estimates (MOE > 50% of estimate)
check_moe <- function(df, label) {
  cat("\nMOE reliability check —", label, "\n")
  result <- df %>%
    select(NAME, starts_with("aian")) %>%
    pivot_longer(-NAME, names_to = "var", values_to = "val") %>%
    mutate(
      suffix  = str_sub(var, -1),
      varname = str_remove(var, "[EM]$")
    ) %>%
    pivot_wider(names_from = suffix, values_from = val) %>%
    filter(!is.na(M) & !is.na(E) & E > 0) %>%
    mutate(moe_pct = round(M / E * 100, 1)) %>%
    filter(moe_pct > 50) %>%
    select(NAME, varname, E, M, moe_pct) %>%
    arrange(desc(moe_pct))
  
  if (nrow(result) == 0) {
    cat("  PASS — No unreliable estimates (MOE > 50%)\n")
  } else {
    cat("  WARNING —", nrow(result), "estimates flagged:\n")
    print(result)
  }
  return(nrow(result))
}
...
missing_summary <- datasets %>%
  imap(function(df, name) {
    n_missing <- df %>% select(ends_with("E")) %>% is.na() %>% sum()
    tibble(dataset = name, n_missing = n_missing)
  }) %>%
  bind_rows()
...
dup_count <- datasets %>%
  imap(function(df, name) {
    ...
    tibble(dataset = name, n_duplicates = n_dup)
  }) %>%
  bind_rows()
...
moe_issues <- 0
moe_issues <- moe_issues + check_moe(labor_2023,   "labor 2023")
moe_issues <- moe_issues + check_moe(labor_2019,   "labor 2019")
moe_issues <- moe_issues + check_moe(labor_2015,   "labor 2015")
moe_issues <- moe_issues + check_moe(poverty_2023, "poverty 2023")
moe_issues <- moe_issues + check_moe(poverty_2019, "poverty 2019")
moe_issues <- moe_issues + check_moe(poverty_2015, "poverty 2015")
moe_issues <- moe_issues + check_moe(housing_2023, "housing 2023")

if (moe_issues == 0) {
  cat("\nMOE SUMMARY: PASS — No unreliable estimates across all datasets\n")
} else {
  cat("\nMOE SUMMARY: WARNING —", moe_issues, "total estimates flagged\n")
}
...
labor_logic <- list(
  "2023" = labor_2023, "2019" = labor_2019, "2015" = labor_2015
) %>%
  imap(function(df, yr) {
    df %>%
      mutate(
...
poverty_logic <- list(
  "2023" = poverty_2023, "2019" = poverty_2019, "2015" = poverty_2015
) %>%
  imap(function(df, yr) {
    df %>%
      mutate(
